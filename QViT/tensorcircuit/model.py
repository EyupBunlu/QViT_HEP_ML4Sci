...
class AttentionHead_Hybrid2(nn.Module):
    def __init__(self,d_t,d_k):
        ...
        self.Q = QLayer(measure_query_key,[3,d_k],int(d_k))
        self.K = QLayer(measure_query_key,[3,d_k],int(d_k))
    ...
    def forward(self,input1):
        ...
        V = self.V(input1)
        Q = self.Q(input1.flatten(start_dim=0,end_dim=1)).reshape(*input1.shape[:2])
        K = self.K(input1.flatten(start_dim=0,end_dim=1)).reshape(*input1.shape[:2])
        A = torch.empty((*input1.shape[:-1],input1.shape[-2]),device=input1.device)
        for j in range(input1.shape[-2]):
            A[...,j] = -(Q-K[...,j][...,None])**2
        return self.attention(A,V)+input1
...
class Transformer(nn.Module):
    def __init__(self,d_t,d_k,n_h,n_layers,attention_type,embed_dim,ff_dim):
...
class HViT(nn.Module):
    def __init__(self,d_t,d_k,n_h,n_layers,FC_layers,attention_type,embed_dim,ff_dim):
        super(HViT,self).__init__()
        self.transformer = Transformer(d_t,d_k,n_h,n_layers,attention_type,embed_dim,ff_dim)
